...
export async function getColleges(): Promise<College[]> {
  ...
  const [collegeRows, listingRows] = await Promise.all([
    ...
    querySupabase<ListingRow[]>("listings?select=id,college_slug,status,category", {
      next: { revalidate: 60 }
    })
  ]);
...
export async function getCollege(slug: string): Promise<College | null> {
  ...
  const [collegeRows, listingRows] = await Promise.all([
    ...
    querySupabase<ListingRow[]>(
      `listings?select=id,category,status&college_slug=eq.${encodeURIComponent(slug)}`,
      {
        next: { revalidate: 60 }
      }
    )
...
export async function getListingsByCategoryBrowse(
  ...
  const listingRows = await querySupabase<ListingRow[]>(
    `listings?select=id,user_id,status,college_slug,title,branch,year,category,condition,price,min_price,expected_price,location,posted_by,description,image,sold_at,sold_delete_at,created_at,colleges(name,city)&category=in.${encodedCategoryFilter}&order=created_at.desc`,
    {
      next: { revalidate: 60 }
    }
  );
...
export async function getRecentListings(limit = 6): Promise<Listing[]> {
  ...
  const listingRows = await querySupabase<ListingRow[]>(
    `listings?select=id,user_id,status,college_slug,title,branch,year,category,condition,price,min_price,expected_price,location,posted_by,description,image,sold_at,sold_delete_at,created_at,colleges(name,city)&order=created_at.desc&limit=${queryLimit}`,
    {
      next: { revalidate: 60 }
    }
  );
